...
εκτελείται ο παρακάτω αλγόριθμος reverse Assignment LSH(η συνθήκη σύγκλησης ελέγχεται μετα την 2 επανάληψη του αλγορίθμου καθώς τότε πλέον θα έχω παλιά κεντροειδή), αναλυτικά:
	...
	-Έπειτα, έχουμε την διαδικασία ανάθεσης κάθε διανύσματος του input στο αντιστοιχό cluster (με βάση τα κεντροειδή). 
	 ...
	 Συγκεκριμένα, κάνουμε αναζήτηση με βάση την δεδομένη ακτίνα (όπως και στην συνάρτηση listFindNeighborsInRadius(...)) μόνο που επιπλέον για 
	 ...
	 τότε αν ισχυει αυτή η συνθήκη ελέγχεται ξανά το διάνυσμα για το αν έχει ανατέθεί ήδη προηγουμένως στο παρόν cluster ή αν εχει ανατεθεί προηγουμένως σε αναζήτηση με διαφορετική 
	 ...
	 φαίνεται ότι ανήκει σε περισσότερα από ένα cluster, συνεπώς το προσθέτω στην λίστα με τα conflicts (confList), ώστε μετά το περας των range search που θα πραγματοποιήθουν 
	 για όλα τα clusters για μια δεδομένη ακτίνα, να αναθέσω αυτό και όποια αλλα διανυσματα είναι στην λίστα με τα conflicts στο cluster που απέχουν την μικρότερη ευκλείδια απόσταση απο το αντίστοιχο κεντροειδή τους.
	 Συνεπώς, αν ΔΕΝ εχει ανατεθεί το διάνυσμα που εξετάζεται σε ένα cluster στην "ίδια επανάληψη" τότε υπολογίζεται η ευκλείδια μεταξυ αυτου και του κεντροειδή (του παρόν cluster) και με την προϋπόθεση	 ότι είναι εντός ακτίνας (η απόσταση απο το κεντροειδη) το διάνυσμα εισάγεται στο αντιστοιχο cluster, μάλιστα στο δεδομένο διάνυσμα καταχωρούνται οι ανάλογες πληροφορίες όσον αναφορά:
	 1) το index του cluster στο οποιο ανατέθηκε 2) τον αριθμό της επάναλήψης 3) την τιμή της ακτίνας που πραγματοποιείται το range search.
